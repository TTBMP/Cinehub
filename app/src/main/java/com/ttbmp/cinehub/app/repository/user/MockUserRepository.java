package com.ttbmp.cinehub.app.repository.user;

import com.ttbmp.cinehub.domain.User;

import java.util.ArrayList;
import java.util.List;

/**
 * @author Fabio Buracchi
 */
public class MockUserRepository implements UserRepository {

    private static final List<UserData> USER_DATA_LIST = new ArrayList<>();

    static {
        USER_DATA_LIST.add(new UserData("-2", "Gastone", "Paperone", "gp@cinehub.com", "customer"));
        USER_DATA_LIST.add(new UserData("-1", "Paperino", "Paolino", "pp@cinehub.com", "manager;customer"));
        USER_DATA_LIST.add(new UserData("0", "Fabio", "Buracchi", "fb@cinehub.com", "usher;employee;customer"));
        USER_DATA_LIST.add(new UserData("1", "Massimo", "Mazzetti", "mm@cinehub.com", "projectionist;employee;customer"));
        USER_DATA_LIST.add(new UserData("2", "Ivan", "Palmieri", "ip@cinehub.com", "usher;employee;customer"));
        USER_DATA_LIST.add(new UserData("3", "Mario", "Rossi", "mr@cinehub.com", "projectionist;employee;customer"));
        USER_DATA_LIST.add(new UserData("5", "Luigi", "Bianchi", "lb@cinehub.com", "projectionist;employee;customer"));
        USER_DATA_LIST.add(new UserData("7", "Steve", "Jobs", "sj@cinehub.com", "projectionist;employee;customer"));
        USER_DATA_LIST.add(new UserData("9", "Bill", "Gates", "bg@cinehub.com", "projectionist;employee;customer"));
        USER_DATA_LIST.add(new UserData("11", "Elon", "Musk", "em@cinehub.com", "projectionist;employee;customer"));
        USER_DATA_LIST.add(new UserData("13", "Alan", "Turing", "at@cinehub.com", "projectionist;employee;customer"));
        USER_DATA_LIST.add(new UserData("15", "James", "Gosling", "jg@cinehub.com", "projectionist;employee;customer"));
        USER_DATA_LIST.add(new UserData("17", "Dennis", "Ritchie", "dr@cinehub.com", "projectionist;employee;customer"));
        USER_DATA_LIST.add(new UserData("19", "Larry", "Page", "lp@cinehub.com", "projectionist;employee;customer"));
        USER_DATA_LIST.add(new UserData("21", "Mark", "Zuckerberg", "mz@cinehub.com", "projectionist;employee;customer"));
        USER_DATA_LIST.add(new UserData("23", "Jeff", "Bezos", "jb@cinehub.com", "projectionist;employee;customer"));
    }


    public static List<UserData> getUserDataList() {
        return USER_DATA_LIST;
    }

    @Override
    public User getUser(String userId) {
        return USER_DATA_LIST.stream()
                .filter(d -> d.id.equals(userId))
                .map(d -> new UserProxy(
                        d.id,
                        d.name,
                        d.surname,
                        d.email,
                        d.roles
                ))
                .findAny()
                .orElse(null);
    }

    public static class UserData {

        private String id;
        private String name;
        private String surname;
        private String email;
        private String roles;

        public UserData(String id, String name, String surname, String email, String roles) {
            this.id = id;
            this.name = name;
            this.surname = surname;
            this.email = email;
            this.roles = roles;
        }

        public String getId() {
            return id;
        }

        public void setId(String id) {
            this.id = id;
        }

        public String getName() {
            return name;
        }

        public void setName(String name) {
            this.name = name;
        }

        public String getSurname() {
            return surname;
        }

        public void setSurname(String surname) {
            this.surname = surname;
        }

        public String getEmail() {
            return email;
        }

        public void setEmail(String email) {
            this.email = email;
        }

        public String getRoles() {
            return roles;
        }

        public void setRoles(String role) {
            this.roles = role;
        }

    }

}
