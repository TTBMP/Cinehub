...
public class ChooseMovieViewController extends ViewController {
    ...
    protected void onLoad() {
        ...
        confirmMovieButton.setOnAction(a -> {
            try {
                activity.getUseCase(BuyTicketUseCase.class).getListCinema(MovieDataMapper.mapToEntity(viewModel.selectedMovieProperty().getValue()));
        ...
        movieListView.setCellFactory(movieList->new ChooseMovieListCell(movieList,activity,navController));
    ...
    private void bind() {
        confirmMovieButton.disableProperty().bind(viewModel.selectedDateProperty().isNull());
